    ...
    def set_semiconductor_parameters(self, op_temperature, tg_region, bl_mat_no, sl_mat_no, doping, ct_doping):
        ...
        if False:
            for each_z in range(1,self.Z_nodes_len-1):
                for each_r in [85]:
                    each_point =tuple([each_r, each_z])
                    for each_neighbor_key in self.CM[each_point].keys():
                        geo = self.CM[each_point][each_neighbor_key]['geometry']
                        print(each_point, each_neighbor_key, geo)
