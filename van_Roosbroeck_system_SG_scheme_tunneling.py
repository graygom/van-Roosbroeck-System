...
if False:
    ...
    for WL_bias in WL_sweep_range:
        ...
        In_bl_array, In_sl_array = [], []
        each_dt_array = [1e-15, 1e-14, 1e-13, 1e-12, 1e-11, 1e-10, 1e-9, 1e-8, 1e-7]

        for each_dt in each_dt_array:
            grid_solver.solve_poisson_equation(model_type='MIS')
            grid_solver.solve_continuity_equation(dt=each_dt, output_filename=False)
            ...
            #
            print('%.2f,%.2e,%.2e' % (WL_bias, each_dt, abs(In_sl)))
        print('')
        
        plt.loglog(each_dt_array, np.abs(In_sl_array),'o-')
        plt.grid(ls=':')
        plt.show()
